 ...
 *
 * The triangulator assumes that the polygon represents a polyline. If
 * the polygon represents a solid shape, it will be ignored.
 *
 ...
 */
void SimpleTriangulator::set(const Poly2& poly) {
    reset();
    if (poly.getGeometry() == Geometry::IMPLICIT) {
        _input = poly.vertices();
    } else if (poly.getGeometry() == Geometry::PATH) {
        for(int ii = 0; ii < poly.indices().size(); ii += 2) {
            _input.push_back(poly.vertices()[poly.indices()[ii]]);
        }
        Uint32 first = poly.indices()[0];
        Uint32 last  = poly.indices()[poly.indices().size()-1];
        if (last != first) {
            _input.push_back(poly.vertices()[last]);
        }
    }
}
...
void SimpleTriangulator::calculate() {
    ...
    // This algorithm uses clockwise as front-facing.  Will reverse later
...
void SimpleTriangulator::trimColinear() {
    int colinear = 0;
    for(int ii = 0; ii < _output.size()/3-colinear; ii++) {
        float t1 = _input[_output[3*ii  ]].x*(_input[_output[3*ii+1]].y-_input[_output[3*ii+2]].y);
        float t2 = _input[_output[3*ii+1]].x*(_input[_output[3*ii+2]].y-_input[_output[3*ii  ]].y);
        float t3 = _input[_output[3*ii+2]].x*(_input[_output[3*ii  ]].y-_input[_output[3*ii+1]].y);
        if (fabs(t1+t2+t3) < 0.0000001f) {
            iter_swap(_output.begin()+(3*ii  ), _output.end()-(3*colinear+3));
            iter_swap(_output.begin()+(3*ii+1), _output.end()-(3*colinear+2));
            iter_swap(_output.begin()+(3*ii+2), _output.end()-(3*colinear+1));
            colinear++;
        }
    }
    
    if (colinear > 0) {
        _output.erase(_output.end()-3*colinear,_output.end());
    }
}

/**
 * Reverses the orientation of the triangles.
 */
void SimpleTriangulator::reverseOrientation() {
    for(size_t ii = 0; ii < _output.size(); ii += 3) {
        Uint32 temp = _output[ii];
        _output[ii] = _output[ii+2];
        _output[ii+2] = temp;
    }
}
 ...
 */
std::vector<Uint32> SimpleTriangulator::getTriangulation() const {
 ...
 */
size_t SimpleTriangulator::getTriangulation(std::vector<Uint32>& buffer) const {
    if (_calculated) {
