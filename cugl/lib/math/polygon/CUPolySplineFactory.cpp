...
//  Author: Walker White
//  Version: 6/22/16
#include <cugl/math/polygon/CUPolySplineFactory.h>
 ...
 */
void PolySplineFactory::calculate(Criterion criterion, float tolerance) {
    reset();
    if (!_spline) { return; }
 ...
 */
int PolySplineFactory::generate(const std::vector<Vec2>& src, int soff, float tp,
                                      float tolerance, PolySplineFactory::Criterion criterion,
                                      int depth) {
    // Do not go to far
    bool terminate = (depth >= 8) || (soff >= src.size() - 1);
        
    // Check if we are at the bottom level
    if (!terminate && criterion == PolySplineFactory::Criterion::SPACING) {
        Vec2 temp0 = src[soff + 3] - src[soff];             // p3 - p0
        terminate = temp0.length() < tolerance;
    }
    else if (!terminate && (criterion == PolySplineFactory::Criterion::DISTANCE ||
                                criterion == PolySplineFactory::Criterion::FLAT)) {
        Vec2 temp0 = src[soff + 3] - src[soff];             // p3 - p0
        float leng = 1.0f;
        if (criterion == PolySplineFactory::Criterion::FLAT) {
            leng = temp0.length();
        }
    ...
    Spline2::subdivide(src, soff, 0.5, left, rght);
 ...
 */
Poly2 PolySplineFactory::getPath() const {
 ...
 */
Poly2* PolySplineFactory::getPath(Poly2* buffer) const {
    ...
    buffer->setGeometry(Geometry::PATH);
    return buffer;
}
 ...
 */
std::vector<float> PolySplineFactory::getParameters() const {
 ...
 */
size_t PolySplineFactory::getParameters(std::vector<float> buffer) {
    if (_calculated) {
 ...
 */
std::vector<Vec2> PolySplineFactory::getTangents() const {
    const std::vector<Vec2>* points = getActivePoints();
 ...
 * @return the number of elements added to the buffer
 */
size_t PolySplineFactory::getTangents(std::vector<Vec2> buffer) {
    const std::vector<Vec2>* points = getActivePoints();
    if (!points) { return 0; }
 ...
 * @return the number of elements added to the buffer
 */
Poly2* PolySplineFactory::getTangents(Poly2* buffer) {
    CUAssertLog(buffer, "Destination buffer is null");
    CUAssertLog(buffer->_geom == Geometry::PATH || buffer->_geom == Geometry::IMPLICIT,
                "Buffer geometry is incompatible with this result.");
    const std::vector<Vec2>* points = getActivePoints();
 ...
 */
std::vector<Vec2> PolySplineFactory::getNormals() const {
    const std::vector<Vec2>* points = getActivePoints();
    std::vector<Vec2> result;
 ...
 * @return the number of elements added to the buffer
 */
size_t PolySplineFactory::getNormals(std::vector<Vec2> buffer) {
    const std::vector<Vec2>* points = getActivePoints();
 ...
 * @return the number of elements added to the buffer
 */
Poly2* PolySplineFactory::getNormals(Poly2* buffer) {
    CUAssertLog(buffer, "Destination buffer is null");
    CUAssertLog(buffer->_geom == Geometry::PATH || buffer->_geom == Geometry::IMPLICIT,
                "Buffer geometry is incompatible with this result.");
    const std::vector<Vec2>* points = getActivePoints();
 ...
 */
Poly2 PolySplineFactory::getAnchors(float radius, int segments) const {
    const std::vector<Vec2>* points = getActivePoints();
    if (!points) { return Poly2(); }
    ...
    poly.setGeometry(Geometry::SOLID);
    return poly;
}
 ...
 * @return a reference to the buffer for chaining.
 */
Poly2* PolySplineFactory::getAnchors(Poly2* buffer, float radius, int segments) const {
    CUAssertLog(buffer, "Destination buffer is null");
    ...
    buffer->setGeometry(Geometry::SOLID);
    return buffer;
}
 ...
 */
Poly2 PolySplineFactory::getHandles(float radius, int segments) const {
    const std::vector<Vec2>* points = getActivePoints();
    if (!points) { return Poly2(); }
 ...
 * @return a reference to the buffer for chaining.
 */
Poly2* PolySplineFactory::getHandles(Poly2* buffer, float radius, int segments) const {
    CUAssertLog(buffer, "Destination buffer is null");
    CUAssertLog(buffer->_geom == Geometry::SOLID || buffer->_geom == Geometry::IMPLICIT,
                "Buffer geometry is incompatible with this result.");
    const std::vector<Vec2>* points = getActivePoints();
 ...
 */
Spline2 PolySplineFactory::getRefinement() const {
    const std::vector<Vec2>* points = getActivePoints();
 ...
 * @return a reference to the buffer for chaining.
 */
Spline2* PolySplineFactory::getRefinement(Spline2* buffer) const {
    CUAssertLog(buffer, "Destination buffer is null");
    const std::vector<Vec2>* points = getActivePoints();
    if (!points) { return buffer; }
