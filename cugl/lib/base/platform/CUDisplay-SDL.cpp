 ...
 * 
 * @return the full screen resolution for this display
 */
 ...
 * size.
 * 
 * @param window	The SDL window context
 *
 * @return the usable full screen resolution for this display
 */
cugl::Rect cugl::impl::DisplaySafeBounds(SDL_Window* window) {
    cugl::Rect result;
    SDL_Rect bounds;
    SDL_GetDisplayBounds(0,&bounds);
    Vec2 scale = DisplayPixelDensity();
    ...
    jmethodID method_id = env->GetStaticMethodID(clazz, "getSafeInsetLeft", "()I");
    insets.x = env->CallStaticIntMethod(clazz, method_id);
    method_id = env->GetStaticMethodID(clazz, "getSafeInsetTop", "()I");
    insets.y = env->CallStaticIntMethod(clazz, method_id);
    method_id = env->GetStaticMethodID(clazz, "getSafeInsetRight", "()I");
    insets.w = env->CallStaticIntMethod(clazz, method_id);
    method_id = env->GetStaticMethodID(clazz, "getSafeInsetBottom", "()I");
    insets.h = env->CallStaticIntMethod(clazz, method_id);
	...
	result.size.width = bounds.w-insets.x-insets.w;
	result.size.width = w < result.size.width ? w : result.size.width;
	result.size.height = bounds.h-insets.y-insets.h;
	result.size.height = h < result.size.height ? h : result.size.height;
	result.origin.x = insets.x;
	result.origin.y = bounds.h-result.size.height-insets.y;
#endif	
