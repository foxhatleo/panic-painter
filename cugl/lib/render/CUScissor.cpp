 ...
 */
Scissor& Scissor::operator=(Scissor&& mask) {
    _inverse   = mask._inverse;
    _transform = mask._transform;
    _bounds  = mask._bounds;
    _fringe  = mask._fringe;
 ...
 *
 * The intersection will take place in the coordinate system of this scissor mask.
 * The other mask will be transformed to be in this coordinate space.  The 
 * transformation of mask will produce a rectangle inscribed in the transformed
 * original. The parameter loose is the same as in the method {@link Rect#inscribe}.
 *
 * @param mask	The scissor mask to intersect with this one
 * @param loose	Whether to loosely inscribe the second mask on transformation
 *
 * @return a reference to the scissor for chaining
 */
Scissor& Scissor::intersect(const Scissor& mask, bool loose) {
    // We need the relative transform
    Affine2 trans = mask._transform*_transform.getInverse();
    Rect temp = mask._bounds;
    temp.inscribe(trans,loose);
    
    // Now intersect in this space.
    temp.intersect(_bounds);
    return set(temp,_transform,_fringe);
}
 ...
 */
Scissor Scissor::getIntersection(const Scissor& mask, bool loose) const {
    Scissor result;
    ...
    // Now intersect in this space.
    temp.intersect(_bounds);
    return result.set(_bounds,_transform,_fringe);
}
 ...
 * @return the intersection of the given scissor mask with this one.
 */
std::shared_ptr<Scissor> Scissor::getIntersection(const std::shared_ptr<Scissor>& mask, bool loose) const {
    // We need the relative transform
    Affine2 trans = mask->_transform*_transform.getInverse();
    Rect temp = mask->_bounds;
    temp.inscribe(trans,loose);
    ...
    // Now intersect in this space.
    temp.intersect(_bounds);
    return Scissor::alloc(temp,_transform,_fringe);
}
...
float* Scissor::getData(float* array) const {
    ...
    array[14] = sqrtf(_inverse.m[0]*_inverse.m[0] + _inverse.m[2]*_inverse.m[2]) / _fringe;
    array[15] = sqrtf(_inverse.m[1]*_inverse.m[1] + _inverse.m[3]*_inverse.m[3]) / _fringe;
    return array;
}
...
float* Scissor::getComponents(float* array) const {
    ...
    array[11] = sqrtf(_inverse.m[0]*_inverse.m[0] + _inverse.m[2]*_inverse.m[2]) / _fringe;
    array[12] = sqrtf(_inverse.m[1]*_inverse.m[1] + _inverse.m[3]*_inverse.m[3]) / _fringe;
    return array;
}
...
std::string Scissor::toString(bool verbose) const {
    ...
    ss << "\n";
    ss << "|  ";
    ss << strtool::to_string(_inverse.m[0]).substr(0,PRECISION);
    ss << ", ";
    ss << strtool::to_string(_inverse.m[2]).substr(0,PRECISION);
    ss << ", ";
    ss << strtool::to_string(_inverse.m[4]).substr(0,PRECISION);
    ss << "  |   |  ";
    ss << strtool::to_string(_bounds.size.width).substr(0,PRECISION);
    ss << "  |";
    ss << "\n";
    ss << "|  ";
    ss << strtool::to_string(_inverse.m[1]).substr(0,PRECISION);
    ss << ", ";
    ss << strtool::to_string(_inverse.m[3]).substr(0,PRECISION);
    ss << ", ";
    ss << strtool::to_string(_inverse.m[5]).substr(0,PRECISION);
    ss << "  |;  |  ";
...
void Scissor::recompute() {
    _inverse.setIdentity();
    _inverse.translate(_bounds.origin+_bounds.size/2);
    _inverse *= _transform;
    _inverse.invert();
}
