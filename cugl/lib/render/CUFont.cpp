 ...
 * character uses the maximum height, it provides the full height
 * of the font.  Furthermore, if the last character does not use the
 * full advance, there will be padding after that character.
 *
 ...
 * the texture that should be used with these vertices. If this is an atlas,
 * the texture will be that atlas.
 *
 * The string may either be in UTF8 or ASCII; the method will handle
 * conversion automatically.  However, by setting the optional value
 * 'utf8' to false, you can speed up the method by skipping the
 ...
 * @param utf8      Whether the string is a UTF8 that must be decoded.
 *
 * @return the texture associated with the mesh
 */
std::shared_ptr<Texture> Font::getMesh(const std::string text, const Vec2 origin,
                                       Mesh<SpriteVertex2>& mesh, bool utf8) {
    Rect bounds(origin,getSize(text));
    if (_hasAtlas) {
        getAtlasMesh(text,origin,bounds,mesh,utf8);
        return _texture;
    }
    
    return getRenderedMesh(text,origin,bounds,mesh,utf8);
}
 ...
 * @return the texture associated with the mesh
 */
std::shared_ptr<Texture> Font::getMesh(const std::string text, const Vec2 origin, const Rect rect,
                                       Mesh<SpriteVertex2>& mesh,  bool utf8) {
    if (_hasAtlas) {
        getAtlasMesh(text,origin,rect,mesh,utf8);
        return _texture;
    }
    
    return getRenderedMesh(text,origin,rect,mesh,utf8);
}
 ...
 * @return the texture associated with the mesh
 */
std::shared_ptr<Texture> Font::getQuad(Uint32 thechar, Vec2& offset, Mesh<SpriteVertex2>& mesh) {
    Rect bounds(offset.x,offset.y, (float)getMetrics(thechar).advance, (float)_fontHeight);
    if (_hasAtlas) {
        getAtlasQuad(thechar,offset,bounds,mesh);
        return _texture;
    }
    
    return getRenderedQuad(thechar,offset,bounds,mesh);
}
...
std::shared_ptr<Texture> Font::getQuad(Uint32 thechar, Vec2& offset, const Rect rect,
    ...
    if (_hasAtlas) {
        getAtlasQuad(thechar,offset,rect,mesh);
        return _texture;
    }
    
    return getRenderedQuad(thechar,offset,rect,mesh);
 ...
 * @param origin    The position of the first character
 * @param mesh      The mesh to store the vertices
 * @param z         The uniform z-offset in 3-d space
 * @param utf8      Whether the string is a UTF8 that must be decoded.
 *
 * @return the texture associated with the mesh
 */
std::shared_ptr<Texture> Font::getMesh(const std::string text, const Vec2 origin,
                                       Mesh<SpriteVertex3>& mesh, float z, bool utf8) {
    Rect bounds(origin,getSize(text));
    if (_hasAtlas) {
        getAtlasMesh(text,origin,bounds,mesh,z,utf8);
        return _texture;
    }
    
    return getRenderedMesh(text,origin,bounds,mesh,z,utf8);
}
...
std::shared_ptr<Texture> Font::getMesh(const std::string text, const Vec2 origin, const Rect rect,
                                       Mesh<SpriteVertex3>& mesh,  float z, bool utf8) {
    if (_hasAtlas) {
        getAtlasMesh(text,origin,rect,mesh,z,utf8);
        return _texture;
    }
    
    return getRenderedMesh(text,origin,rect,mesh,z,utf8);
}
 ...
 * @return the texture associated with the mesh
 */
std::shared_ptr<Texture> Font::getQuad(Uint32 thechar, Vec2& offset, Mesh<SpriteVertex3>& mesh, float z) {
    Rect bounds(offset.x,offset.y, (float)getMetrics(thechar).advance, (float)_fontHeight);
    if (_hasAtlas) {
        getAtlasQuad(thechar,offset,bounds,mesh,z);
        return _texture;
    }
    
    return getRenderedQuad(thechar,offset,bounds,mesh,z);
}
 ...
 */
std::shared_ptr<Texture> Font::getQuad(Uint32 thechar, Vec2& offset, const Rect rect,
                                       Mesh<SpriteVertex3>& mesh, float z) {
    if (_hasAtlas) {
        getAtlasQuad(thechar,offset,rect,mesh,z);
        return _texture;
    }
    
    return getRenderedQuad(thechar,offset,rect,mesh,z);
 ...
 *
 * This method will append the vertices to the provided mesh and update
 * the indices to include these new vertices. This method does not return
 * anything because the quads use the atlas.
 *
 * The string may either be in UTF8 or ASCII; the method will handle
 * conversion automatically.  However, by setting the optional value
 * 'utf8' to false, you can speed up the method by skipping the
 ...
 * @param utf8      Whether the string is a UTF8 that must be decoded.
 */
void Font::getAtlasMesh(const std::string text, const Vec2 origin, const Rect rect,
                        Mesh<SpriteVertex2>& mesh, bool utf8) {
    ...
    if (!utf8) {
        for(int ii = 0; ii < line.size(); ii++) {
            ...
            ii = (getAtlasQuad(line[ii],offset,rect,mesh) ? ii+1 : (int)line.size());
        }
        return;
    }
    ...
    for(int ii = 0; ii < utf32.size();) {
        ...
        ii = (getAtlasQuad(utf32[ii],offset,rect,mesh) ? ii+1 : (int)utf32.size());
    }
}
 ...
 */
std::shared_ptr<Texture> Font::getRenderedMesh(const std::string text, const Vec2 origin,
                                               const Rect rect, Mesh<SpriteVertex2>& mesh, bool utf8) {
 ...
 * Creates a single quad to render this character and stores it in the mesh
 *
 * This method will append the vertices to the given mesh and update
 * the indices to include these new vertices. The quad is adjusted so that
 * all of the vertices fit in the provided rectangle. This may mean that no
 * quad is generated at all.
 *
 ...
 */
bool Font::getAtlasQuad(Uint32 thechar, Vec2& offset, const Rect rect, Mesh<SpriteVertex2>& mesh) {
    CUAssertLog(mesh.command == GL_TRIANGLES, "The mesh is not formatted for triangles");
...
void Font::getAtlasMesh(const std::string text, const Vec2 origin, const Rect rect,
    ...
    if (!utf8) {
        for(int ii = 0; ii < line.size(); ii++) {
            ...
            ii = (getAtlasQuad(line[ii],offset,rect,mesh,z) ? ii+1 : (int)line.size());
        }
        return;
    }
    ...
    for(int ii = 0; ii < utf32.size();) {
        ...
        ii = (getAtlasQuad(utf32[ii],offset,rect,mesh,z) ? ii+1 : (int)utf32.size());
    }
}
 ...
 */
std::shared_ptr<Texture> Font::getRenderedMesh(const std::string text, const Vec2 origin,
                                               const Rect rect, Mesh<SpriteVertex3>& mesh, float z, bool utf8) {
    CUAssertLog(mesh.command == GL_TRIANGLES, "The mesh is not formatted for triangles");
    SDL_Color color;
    color.r = color.g = color.b = color.a = 255;
    ...
    // Bottom left
    temp.position.x = quad.origin.x;
    temp.position.y = quad.origin.y;
    temp.position.z = z;
    temp.color = Color4::WHITE;
    temp.texcoord.x = (quad.getMinX()-origin.x)/surf1->w;
    temp.texcoord.y = 1-(quad.getMinY()-origin.y)/surf1->h;
    ...
    // Bottom right
    temp.position.x = quad.origin.x+quad.size.width;
    temp.position.y = quad.origin.y;
    temp.position.z = z;
    temp.color = Color4::WHITE;
    temp.texcoord.x = (quad.getMaxX()-origin.x)/surf1->w;
    ...
    // Top right
    temp.position.x = quad.origin.x+quad.size.width;
    temp.position.y = quad.origin.y+quad.size.height;
    temp.position.z = z;
    temp.color = Color4::WHITE;
    temp.texcoord.x = (quad.getMaxX()-origin.x)/surf1->w;
    temp.texcoord.y = 1-(quad.getMaxY()-origin.y)/surf1->h;
 ...
 * @return true if the right edge of the glyph was generated
 */
bool Font::getAtlasQuad(Uint32 thechar, Vec2& offset, const Rect rect, Mesh<SpriteVertex3>& mesh, float z) {
    CUAssertLog(mesh.command == GL_TRIANGLES, "The mesh is not formatted for triangles");
