...
void Slider::placeKnob(const std::shared_ptr<Button>& knob) {
    ...
    if (knob == nullptr) {
        ...
        PolyFactory factory;
        factory.setGeometry(Geometry::SOLID);
        factory.setSegments(KNOB_SEGS);
...
void Slider::placePath(const std::shared_ptr<scene2::SceneNode>& path) {
    ...
    if (path == nullptr) {
        ...
        PolyFactory factory;
        Poly2 poly = factory.makeLine(_bounds.origin,_bounds.origin+_bounds.size);
        auto track = PathNode::allocWithPoly(poly,radius,
                                             poly2::Joint::NONE,
                                             poly2::EndCap::ROUND);
        ...
        auto line = PathNode::allocWithPoly(poly,LINE_WEIGHT,
                                            poly2::Joint::NONE,
                                            poly2::EndCap::ROUND);
