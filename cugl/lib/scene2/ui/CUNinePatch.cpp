...
//  Version: 11/8/17
//
#include <cugl/scene2/ui/CUNinePatch.h>
 ...
 */
bool NinePatch::initWithData(const Scene2Loader* loader, const std::shared_ptr<JsonValue>& data) {
    if (_texture != nullptr) {
        CUAssertLog(false, "NinePatch is already initialized");
        return false;
    } else if (!data) {
...
std::string NinePatch::toString(bool verbose) const {
    ...
    ss << ", name:" << _name;
    ss << ", texture:";
    ss <<  cugl::strtool::to_string(texid);
    ss << ")";
    if (verbose) {
        ss << "\n";
 ...
 */
Uint32 NinePatch::generatePatch(const Rect src, const Rect dst, Uint32 offset) {
    ...
    temp.texcoord.y = 1-(src.origin.y+src.size.height)/tsize.height;
    _mesh.vertices.push_back(temp);
    _mesh.indices.push_back(offset+2);
    _mesh.indices.push_back(offset);
    _mesh.indices.push_back(offset+2);
    ...
    temp.texcoord.y = 1-src.origin.y/tsize.height;
    _mesh.vertices.push_back(temp);
    _mesh.indices.push_back(offset+3);
