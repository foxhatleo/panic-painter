...
//  Version: 8/20/20
//
#include <cugl/scene2/ui/CUProgressBar.h>
...
bool ProgressBar::initWithCaps(const std::shared_ptr<Texture>& background,
    ...
    std::shared_ptr<Texture> temp = (background == nullptr ? Texture::getBlank() : background);
    ...
    if (foreground != nullptr) {
        ...
        std::shared_ptr<Texture> temp = Texture::getBlank();
 ...
 */
bool ProgressBar::initWithData(const Scene2Loader* loader, const std::shared_ptr<JsonValue>& data) {
    if (!data) {
        return init();
