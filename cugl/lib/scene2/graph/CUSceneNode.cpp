 ...
 */
SceneNode* SceneNode::copy(SceneNode* dst) {
 ...
 */
void SceneNode::addChild(const std::shared_ptr<SceneNode>& child, int zval) {
...
void SceneNode::setZDirty(bool value) {
    _zDirty = value;
    if (value) {
        if (_parent != nullptr) {
 ...
 */
bool SceneNode::compareNodeSibs(const std::shared_ptr<SceneNode>& a,
 ...
 */
void SceneNode::sortZOrder() {
    if (_zDirty) {
        std::sort(_children.begin(),_children.end(),SceneNode::compareNodeSibs);
        // Fix the offsets
        int ii = 0;
        for(auto it = _children.begin(); it != _children.end(); ++it ) {
            (*it)->_childOffset = ii++;
 ...
 */
void SceneNode::render(const std::shared_ptr<SpriteBatch>& batch, const Mat4& transform, Color4 tint) {
    ...
    if (_scissor) {
        ...
        if (active) {
            local = active->getIntersection(local, false);
        }
