...
void TexturedNode::generateRenderData() {
    ...
    for(size_t ii = 0; ii < _polygon.vertices().size(); ii++) {
        const Vec2 pos = _polygon.vertices()[ii];
        ...
        _mesh.vertices[ii].texcoord.x = s*_texture->getMaxS()+(1-s)*_texture->getMinS();
        _mesh.vertices[ii].texcoord.y = t*_texture->getMaxT()+(1-t)*_texture->getMinT();

        if (_gradient) {
            s = pos.x/_polygon.getBounds().size.width;
            t = pos.y/_polygon.getBounds().size.height;

            if (_flipHorizontal) { s = 1-s; }
            if (!_flipVertical)  { t = 1-t; }
            _mesh.vertices[ii].color = Vec4(s,t,0,0);
        }
    }
