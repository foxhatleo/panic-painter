 ...
 */
void ObstacleSelector::setDebugScene(const std::shared_ptr<scene2::SceneNode>& node) {
...
void ObstacleSelector::updateTarget(Obstacle* obstacle) {
    if (obstacle) {
        ...
        if (_connect != nullptr) {
            ...
            Poly2 poly = factory.makeLine(pos1,pos2);
...
Poly2 ObstacleSelector::hatchPoly() const {
    ...
    Uint32 indx[12] = { 0,3,3,1,1,2,2,0,0,1,2,3 };
    
    Poly2 poly(reinterpret_cast<float*>(verts),4,indx,12);
