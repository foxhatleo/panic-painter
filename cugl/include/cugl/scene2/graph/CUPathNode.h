...
#pragma mark Constructors
     ...
     * First it will traverse the vertices according to the rule of this
     * node (e.g. is it closed or open). Then it will extrude that polygon
     * with the current joint and cap. This method uses {@link SimpleExtruder},
     * as it is safe for framerate calculation.
     *
    ...
    bool initWithVertices(const std::vector<Vec2>& vertices, float stroke,
                          ...
                          poly2::EndCap cap = poly2::EndCap::NONE,
                          bool closed=true);
     ...
     * color.
     *
     * The polygon must have geometry {@link Geometry#IMPLICIT} or
     * {@link Geometry#PATH}.  An `IMPLICIT` geometry will be considered
     * closed and traversed accordingly. However, if  the geometry is
     * `PATH`, then the polygon will remain as given and the traversal
     * (closed or open) will be inferred from the indices.
     *
     * Once the traversal of the polygon is finalized, this method will e
     * extrude that polygon with the current joint and cap. This method
     * uses {@link SimpleExtruder}, as it is safe for framerate calculation.
     *
...
#pragma mark Polygons
     ...
     *
     * The polygon must have geometry {@link Geometry#IMPLICIT} or
     * {@link Geometry#PATH}.  An `IMPLICIT` geometry will be traversed
     * according to whehter this node is closed or open.  If the geometry
     * is `PATH`, then the polygon will remain as given if this node is
     * open, but it will be smoothed (removing all gaps) if it is closed.
     *
     * Once the traversal of the polygon is finalized, this method will
     * extrude that polygon with the current joint and cap. This method
     * uses {@link SimpleExtruder}, as it is safe for framerate calculation.
     *
     ...
     *
     * The rectangle will be converted into a Poly2, using the standard outline.
     * However, the closure of the ends will depend upon the current setting
     * of this node.  An open rectangle is open on the left side.
     *
     * Once the traversal of the rectangle is finalized, this method will
     * extrude that rect with the current joint and cap. This method uses
     * {@link SimpleExtruder}, as it is safe for framerate calculation.
     *
...
private:
     ...
     */
    bool canonicallyClosed(const Poly2& poly) const;
