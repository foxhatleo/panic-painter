...
#ifndef __CU_SCENE_NODE_H__
#define __CU_SCENE_NODE_H__
 ...
 *
 * Nodes do support optional z-ordering. This is not a true depth value, as
 * depth filtering is incompatible with alpha compositing. However, it does
 * provide a way to dynamically reorder how children are composed.
 *
...
#pragma mark Visibility
     ...
     *
     * SceneNodes are just coordinate spaces. Any child outside of the
     * content area is still drawn; the content area value is primarily
     * used for anchor placement. If you want to obscure (parts of)
     * children that are outside the content area, you will need to set
     * a scissor.
     *
     * The scissor does not need to be set to the content area. It can
     * by any scissor, and that scissor will be applied to the coordinate
     * system of this node. To create a scissor that will restrict to
     * the node content area, simply construct
     *
     *     Scissor::alloc(getContentSize())
     *
     * It is possible for the children of a node to have a scissor as
     * well.  When that happens, the children will be rendered with
     * the intersection of the two scissors. However, if the child
     * is rotated, then its scissor will be oriented to be aligned
     * with the parent scissor, as we can only intersect two rectangles
     * of the same orientation. The rule for this intersection will
     * be the same as {@link Scissor#intersect}.
     *
     * @return the scissor associated with this node.
     */
    std::shared_ptr<Scissor> getScissor() const { return _scissor; }
     ...
     * be the same as {@link Scissor#intersect}.
     *
     * @param scissor   The scissor associated with this node.
     */
    void setScissor(const std::shared_ptr<Scissor>& scissor) { _scissor = scissor;}
     ...
     * a scissor.
     *
     * The scissor set by this method is guaranteed to fit the
     * content bounds ({@see #getContentSize()}) of this node. However,
     * it will not be updated if the content bounds of the node are
     * changed. In that case, this method should be called again.
     *
     * It is possible for the children of a node to have a scissor as
     * well. When that happens, the children will be rendered with
     * the intersection of the two scissors. However, if the child
     ...
     * be the same as {@link Scissor#intersect}.
     */
    void setScissor() { _scissor = Scissor::alloc(getContentSize());}
...
#pragma mark Z-order
     ...
     *
     * Sorting does not happen automatically (except within a {@link Scene2}).
     ...
     * associated {@link Scene2}). Our methods guarantee this invariant, so
...
#pragma mark Scene Graph
     ...
     * @return a (weak) pointer to the scene graph.
     */
    const Scene2* getScene() const { return _graph; }
...
#pragma mark Internal Helpers
     ...
     */
    void setScene(Scene2* scene) { _graph = scene; }
     ...
     */
    static bool compareNodeSibs(const std::shared_ptr<SceneNode>& a, const std::shared_ptr<SceneNode>& b);
    ...
    friend class cugl::Scene2;
};
    }

}
#endif /* __CU_SCENE_NODE_H__ */
